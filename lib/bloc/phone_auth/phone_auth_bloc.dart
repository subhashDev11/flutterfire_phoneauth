...
import 'package:flutter_test_project/data/repository/phone_auth_repository.dart';
...
class PhoneAuthBloc extends Bloc<PhoneAuthEvent, PhoneAuthState> {
  ...
  PhoneAuthBloc({required this.phoneAuthRepository})
    ...
    on<PreLoggedInEvent>((event, emit) => emit(PhoneAuthVerified(event.user)));
    on<LogoutEvent>((event, emit) => emit(PhoneAuthInitial()));
  }

  void init() {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      add(PreLoggedInEvent(user: user));
    }
  }
